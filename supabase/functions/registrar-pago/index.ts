...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (clienteEmail) {
      console.log(`[registrar-pago] Intento envío correo cliente (${clienteEmail}) pago ${pago.id}`)
      try {
    ...
    if (adminEmail) {
      console.log(`[registrar-pago] Intento envío correo admin (${adminEmail}) pago ${pago.id}`)
      try {
