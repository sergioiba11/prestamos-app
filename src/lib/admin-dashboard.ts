...
export async function fetchAdminPanelData() {
  const [clientesRes, prestamosRes, cuotasRes, pagosRes] = await Promise.all([
    supabase.from('clientes').select('id,nombre,dni,telefono,usuario_id,usuarios:usuario_id(email)'),
    supabase
      .from('prestamos')
    ...
    supabase
      .from('cuotas')
      .select('prestamo_id,numero_cuota,fecha_vencimiento,monto_cuota,saldo_pendiente,estado')
      .order('numero_cuota', { ascending: true }),
    supabase.from('pagos').select('id,cliente_id,prestamo_id,monto,metodo,created_at,fecha_pago').order('created_at', { ascending: false }),
  ])
  ...
  for (const cuota of cuotas) {
    ...
    cobrarHoy += Number(cuota.saldo_pendiente || cuota.monto_cuota || 0)
  }
  ...
  const prestamosVencidos = prestamos.filter((p) => {
    ...
    if (p.fecha_limite && isActivoEstado(p.estado)) {
  ...
  const pagosPendientesRaw = pagos.filter((p) => !p.fecha_pago)
  ...
  const activosCards: ClientePrestamoActivo[] = activePrestamos.map((prestamo) => {
    ...
    return {
      ...
      prestamoActivo: Number(prestamo.total_a_pagar || prestamo.monto || 0),
  ...
  return { kpis, activosCards, pagosPendientesList }
}
