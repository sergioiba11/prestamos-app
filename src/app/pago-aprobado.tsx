...
export default function PagoAprobado() {
  ...
  const montoAplicadoParam = numberFromParam(params.monto)
  const montoIngresadoParam = numberFromParam(params.monto_ingresado)
  const vueltoParam = numberFromParam(params.vuelto)
  const montoAplicado = Number.isFinite(Number(pago?.monto))
    ? Number(pago?.monto || 0)
    : Number(montoAplicadoParam || 0)
  const montoIngresado = Number.isFinite(Number(montoIngresadoParam))
    ? Number(montoIngresadoParam || 0)
    : montoAplicado
